...
{	
	var $order = 2;
	var $tab = "tab_empresas";
	var $nombre = "Empresa";
	var $nombres = "Empresas";
	var $mensaje = "la empresa";
	var $columnas = array( "Código", "Nombre", "Llave" );
	var $campos = array( "codEmpresa", "nomEmpresa", "keyEmpresa" );
	...
	{
		$this -> conexion -> Start();
		$_POST[usrCreacion] = $_SESSION[SESSION][session_user][codUsuario];		
	...
	{		
		$this -> conexion -> Start();
		$_POST[codUsuario] = $_SESSION[SESSION][session_user][codUsuario];	
	...
	function Vista( $titulo , $boton , $state )
	{
		...
		$form -> Label( array( "label" => $this -> columnas[2], "for" => $this -> campos[2] ) );
		$form -> TextField( array( "maxlenght" => "100", "size" => "30" , "name" => $this -> campos[2],  "value" => $_POST[$this -> campos[2]] ) );
		$form -> closeRow();
		$form -> closePanel();	
